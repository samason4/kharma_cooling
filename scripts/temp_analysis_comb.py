...
# python3 ./scripts/temp_analysis_comb.py
...
#**********************************************************************************************************************
    ...
    dump_with = fluid_dump.load_dump("./dumps_without/torus.out0.{0:05}.phdf".format(dumpval))
    ...
    ax0.annotate('Log(Temp in K) Iharm3d on top, Kharma on bottom, both with only heating at t ~ '+str(t),xy=(0.5,0.5),xycoords='axes fraction',va='center',ha='center',fontsize='x-large')
    ax0.axis("off")
#****************************************************************************************************************************************************************************************************************************************************************************************************************************
    #plotting rho:
    ax1 = fig.add_subplot(gs[1,0])
    rhopolplot = ax1.pcolormesh(xp, zp, rhop, cmap=cmap, vmin=-2, vmax=0, shading=shading)
    ...
    rho2polplot = ax2.pcolormesh(xp2k, zp2k, rhop2, cmap=cmap, vmin=-2, vmax=0, shading=shading)
    ...
    uupolplot = ax11.pcolormesh(xp, zp, uup, cmap=cmap, vmin=-5, vmax=-2, shading=shading)
    ...
    uu2polplot = ax12.pcolormesh(xp2k, zp2k, uup2, cmap=cmap, vmin=-5, vmax=-2, shading=shading)
...
#****************************************************************************************************************************************************************************************************************************************************************************************************************************
    ...
    plt.tight_layout()
    plt.savefig("temp_plot_v5_{0:03}".format(dumpval))
    plt.close()
...
if __name__=="__main__":
    ...
    dend = 599#int(sorted(os.listdir('./dumps_without'))[-1][11:16])
    dlist = range(dstart,dend+1)

    # Setting grid dict
    dumpk = fluid_dump.load_dump("./dumps_without/torus.out0.{0:05}.phdf".format(0))
    ...
    dfile = h5py.File(os.path.join('dumps_iharm_3d_without','dump_0000{0:04d}.h5'.format(dstart)),'r')
